...
Pcl_Clustering::Pcl_Clustering() : Node("PCL_Clustering") {
  ...
  this->declare_parameter<int>("coloring_distance", 0);
...
void Pcl_Clustering::setup() {
  ...
  RCLCPP_INFO(this->get_logger(), "min_cluster_size: %.2f", min_cluster_size);
  RCLCPP_INFO(this->get_logger(), "max_cluster_size: %.2f", max_cluster_size);
  ...
  cluster_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("/cluster_points", 10);
...
void Pcl_Clustering::pub_cluster(){
  ...
  pointcloud_pub->publish(cluster_msg);
}
